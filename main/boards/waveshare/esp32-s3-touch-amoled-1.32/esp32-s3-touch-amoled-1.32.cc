...
class CustomBoard : public WifiBoard {
  private:
    ...
    void InitializeTools() {
        ...
        mcp_server.AddTool("self.disp.setbacklight", "Set screen brightness", PropertyList({Property("level", kPropertyTypeInteger, 0, 255)}), [this](const PropertyList &properties) -> ReturnValue {
        ...
        mcp_server.AddTool("self.disp.network", "Reconfigure network", PropertyList(), [this](const PropertyList &) -> ReturnValue {
            EnterWifiConfigMode();
            return true;
        });
    }
