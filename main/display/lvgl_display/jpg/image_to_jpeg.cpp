...
static uint8_t* convert_input_to_encoder_buf(const uint8_t* src, uint16_t width, uint16_t height, v4l2_pix_fmt_t format,
    ...
    // GRAY is fed directly as JPEG_PIXEL_FORMAT_GRAY input
    ...
    // V4L2 YUYV (Y Cb Y Cr) can be fed directly as JPEG_PIXEL_FORMAT_YCbYCr input
    ...
    // V4L2 UYVY (Cb Y Cr Y) -> reorder to YUYV and feed as YCbYCr
    // The current version does not encounter UYVY format for now
    ...
    // V4L2 YUV422P (YUV422 Planar) -> reorder to YUYV (YCbYCr)
    // The current version does not encounter YUV422P format for now
    ...
    // Convert RGB to YUV422 (YCbYCr) before feeding
    // See https://github.com/78/xiaozhi-esp32/issues/1380#issuecomment-3497156378
        ...
        switch (format) {
            ...
            case V4L2_PIX_FMT_RGB565:
                ...
                break;
            [[unlikely]] case V4L2_PIX_FMT_RGB565X: // The current version does not encounter RGB565X for now
...
static uint8_t* convert_input_to_hw_encoder_buf(const uint8_t* src, uint16_t width, uint16_t height, v4l2_pix_fmt_t format,
    ...
    if (format == V4L2_PIX_FMT_YUYV) {
        // Hardware requires "big-endian" | Y1 V Y0 U | format, so bswap16 is needed
        int sz = (int)width * (int)height * 2;
...
static bool encode_with_esp_new_jpeg(const uint8_t* src, size_t src_len, uint16_t width, uint16_t height,
    ...
    // Estimate output buffer: 1.5x width*height + 64KB
    size_t out_cap = (size_t)width * (size_t)height * 3 / 2 + 64 * 1024;
    if (out_cap < 128 * 1024)
        out_cap = 128 * 1024;
