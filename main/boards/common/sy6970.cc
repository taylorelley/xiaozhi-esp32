...
int Sy6970::GetBatteryLevel() {
    int level = 0;
    // Lowest voltage the battery can discharge to
    ...
    // Charging status is unstable when the battery is not connected, and battery_voltage sometimes exceeds charge_voltage_limit
    if (level > 100) {
        level = 100;
    }
    return level;
