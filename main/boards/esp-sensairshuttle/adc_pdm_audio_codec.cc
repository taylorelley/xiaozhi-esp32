...
AdcPdmAudioCodec::AdcPdmAudioCodec(int input_sample_rate, int output_sample_rate,
    ...
    if(pdm_speak_n != GPIO_NUM_NC){
        ...
        esp_rom_gpio_connect_out_signal(pdm_speak_n, I2SO_SD_OUT_IDX, 1, 0); //Invert output SD OUT signal
...
void AdcPdmAudioCodec::EnableOutput(bool enable) {
    ...
    if (enable) {
        ...
        // Force PDM TX clock reconfiguration per board config, overriding the third-party library's default up_sample_fs in set_fmt
        // If the channel is already enabled, disable, reconfigure, then re-enable
