...
void WifiConfigUI::StartScanning() {
    ...
    if (scan_results_.empty()) {
        lv_obj_clean(canvas);
        DrawHeader("No Wi-Fi found");
        DrawFooter("W: Manual entry  Esc: Exit");
    } else {
...
void WifiConfigUI::RedrawPasswordInput() {
    ...
    lv_label_set_text(pwd_label, "Please enter password:");
...
void WifiConfigUI::RedrawManualInput() {
    ...
    DrawHeader("Manually configure Wi-Fi");
    ...
    DrawFooter("Tab: Toggle  Enter: Confirm  Esc: Back");
}
...
void WifiConfigUI::DrawSavedWifiList() {
    ...
    char title[48];
    snprintf(title, sizeof(title), "Saved Wi-Fi (%d/10)", (int)saved_wifi_list_.size());
    DrawHeader(title);

    if (saved_wifi_list_.empty()) {
        lv_obj_t* empty_label = lv_label_create(canvas);
        lv_label_set_text(empty_label, "No saved Wi-Fi");
...
void WifiConfigUI::ShowConnecting() {
    ...
    lv_obj_t* ssid_label = lv_label_create(canvas);
    lv_label_set_text_fmt(ssid_label, "Connecting: %s", selected_ssid_.c_str());
...
void WifiConfigUI::ShowSuccess() {
    ...
    lv_label_set_text(saved_label, "Wi-Fi configuration saved");
...
void WifiConfigUI::ShowFailed() {
    ...
    lv_obj_t* ssid_label = lv_label_create(canvas);
    lv_label_set_text_fmt(ssid_label, "Unable to connect: %s", selected_ssid_.c_str());
