...
void ElectronEmojiDisplay::InitializeElectronEmojis() {
    ESP_LOGI(TAG, "Electron emoji initialization will be handled by the Assets system");
    // Emoji initialization has been moved to the assets system, configured via DEFAULT_EMOJI_COLLECTION=otto-gif
    // assets.cc loads GIF emojis from the assets partition and sets them into the theme
...
void ElectronEmojiDisplay::SetStatus(const char* status) {
    ...
    if (strcmp(status, Lang::Strings::LISTENING) == 0) {
        ...
        lv_obj_set_style_text_font(status_label_, &OTTO_ICON_FONT, 0);
        lv_label_set_text(status_label_, "\xEF\x80\xA8");  // U+F028 speaking icon
        lv_obj_clear_flag(status_label_, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(network_label_, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(battery_label_, LV_OBJ_FLAG_HIDDEN);
        return;
        ...
        lv_obj_set_style_text_font(status_label_, &OTTO_ICON_FONT, 0);
        lv_label_set_text(status_label_, "\xEF\x83\x81");  // U+F0c1 connect icon
        lv_obj_clear_flag(status_label_, LV_OBJ_FLAG_HIDDEN);
        return;
