...
private:
    ...
    // power
    bool pressed = false;                   // Whether the power button is pressed
    int PowerControl_ticks_ = 0;            // Power-on duration
    int press_ticks_ = 0;                   // Tick count when pressed
    bool is_first_boot = true;              // 
    uint8_t PowerDec_level_ = 0;            // Power-button level
    const int power_off_ticks_ = 20;        // Prepare shutdown after button held for 20/5 seconds;
    bool new_charging_status = false;       // Software shutdown unavailable while USB is plugged in
    bool is_shutting_down_ = false;  // Marker for entering the shutdown sequence
    int shutdown_delay_ticks_ = 0;   // Shutdown delay count
    uint8_t shutdown_ticks = 5;      // 5/5sReserved shutdown operation to run afterwards
    bool shutdown_first_ = true;

    // Software shutdown logic for legacy hardware
    void PowrSwitch() {
        PowerDec_level_ = gpio_get_level(Power_Dec);

        // Ensure the power button is released after power-on before checking for shutdown
    ...
    void ReadBatteryAdcData() {
        ...
        // Add the ADC value to the queue
        ...
        // Define battery capacity ranges
        const struct {
            uint16_t adc;
            uint8_t level;
        } levels[] = {
...
public:
    PowerManager(gpio_num_t pin) : charging_pin_(pin) {
        ...
        // Create power control check timer
        ...
        ESP_ERROR_CHECK(adc_oneshot_config_channel(adc_handle_, POWER_BATTERY_ADC_CHANNEL, &chan_config)); // Battery level
    ...
    void shutdown() {
        ...
        {
            shutdown_first_ = false; // Set to false after entering, to prevent re-entering the shutdown state
            ...
            for (int i=1;i<15;i++) {
                ...
                vTaskDelay(pdMS_TO_TICKS(100));
                ESP_LOGI("PowerManager","Trigger power on/off control");
            }
            ESP_LOGI("PowerManager","Shutdown failed, entering deep sleep");
            esp_deep_sleep_start();
        } else {
            ESP_LOGI("PowerManager","USB plug detected, cannot shut down"); 
        }
    }
};
