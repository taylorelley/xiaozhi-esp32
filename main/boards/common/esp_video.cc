...
EspVideo::EspVideo(const esp_video_init_config_t& config) {
    ...
    // Note: in the current version of esp_video, YUV422P actually outputs YUYV.
    ...
    auto get_rank = [](uint32_t fmt) -> int {
        switch (fmt) {
            ...
            case V4L2_PIX_FMT_YUV420:  // The software JPEG encoder does not support YUV420 format
                return 2;
    ...
    // When ISP is enabled, the ISP needs some images to initialize parameters, so after starting it, capture and discard images in the background for 5s
    xTaskCreate(
        [](void* arg) {
...
bool EspVideo::Capture() {
        ...
        if (i == 2) {
            ...
            switch (sensor_format_) {
                ...
                case V4L2_PIX_FMT_RGB565X: {
                    ...
                    // Currently esp_video returns RGB565 for both big- and little-endian and never returns RGB565X; this case exists for future-version compatibility
                    auto src16 = (uint16_t*)mmap_buffers_[buf.index].start;
                    auto dst16 = (uint16_t*)frame_.data;
    ...
    if (display != nullptr) {
        ...
        size_t stride = ((w * 2) + 3) & ~3;  // 4-byte aligned
        lv_color_format_t color_format = LV_COLOR_FORMAT_RGB565;
        uint8_t* data = nullptr;

        switch (frame_.format) {
            // LVGL seems to have problems displaying YUV-family images, so convert to RGB565 for now
            case V4L2_PIX_FMT_YUYV:
            case V4L2_PIX_FMT_YUV420:
            case V4L2_PIX_FMT_RGB24: {
            ...
            case V4L2_PIX_FMT_RGB565:
                ...
                lvgl_image_size = frame_.len;  // On fallthrough, accommodates both YUYV and RGB565
                break;
 ...
 *
 * This function encodes the image in the current camera buffer as JPEG and sends it
 * via an HTTP POST request as multipart/form-data to the configured explain server.
 * The server performs AI analysis of the image based on the provided question and
 * returns the result.
 *
 * Implementation notes:
 * - Encodes JPEG on a separate thread, decoupled from the main thread
 * - Uses chunked transfer encoding to optimize memory usage
 * - Synchronizes the encoder thread and sender thread via a queue
 * - Supports configuring the Device-Id, Client-Id, and authentication-token HTTP headers
 *
 * @param question The question about the image to ask the AI; sent as a form field
 * @return std::string JSON-formatted response string returned by the server
 *         On success contains the AI analysis result; on failure contains an error message
 *         Example format: {"success": true, "result": "analysis result"}
 *                        {"success": false, "message": "error message"}
 *
 * @note SetExplainUrl() must be called to configure the server URL before this function
 * @note This function waits for the previous encoder thread to finish before starting new work
 * @warning If the camera buffer is empty or the network connection fails, an error is returned
 */
std::string EspVideo::Explain(const std::string& question) {
    ...
    // Configure the HTTP client to use chunked transfer encoding
    ...
    {
        // Second chunk: file-field header
    ...
    {
        // Fourth chunk: multipart trailer
