...
private:
    ...
    bool DetectHardwareVersion() {
        ...
        for (size_t i = 0; i < sizeof(camera_addresses); i++) {
            ...
            if (ret == ESP_OK) {
                ...
                if (ret == ESP_OK && (data[0] != 0 || data[1] != 0)) {
                    ...
                    ESP_LOGI(TAG, "Detected camera (OV2640 mode) PID=0x%04X (address=0x%02X)",
                             detected_pid, addr);
                    camera_found = true;
                ...
                if (ret == ESP_OK) {
                    ...
                    if (ret == ESP_OK) {
                        ...
                        if (detected_pid != 0) {
                            ESP_LOGI(TAG, "Detected camera (OV3660 mode) PID=0x%04X (address=0x%02X)",
                                     detected_pid, addr);
                            camera_found = true;
                            i2c_master_bus_rm_device(dev_handle);
                            break;
        ...
        if (!camera_found) {
            ...
            // Determine camera type by PID
            if (detected_pid == OV2640_PID_1 || detected_pid == OV2640_PID_2) {
                camera_type_ = OTTO_CAMERA_OV2640;
                ESP_LOGI(TAG, "Camera type: OV2640 (PID=0x%04X)", detected_pid);
            } else if (detected_pid == OV3660_PID) {
                camera_type_ = OTTO_CAMERA_OV3660;
                ESP_LOGI(TAG, "Camera type: OV3660 (PID=0x%04X)", detected_pid);
            } else {
                camera_type_ = OTTO_CAMERA_UNKNOWN;
                ESP_LOGW(TAG, "Unknown camera type, PID=0x%04X", detected_pid);
            }
        }
        return camera_found;
    }
...
private:
    ...
    bool InitializeCamera() {
        ...
        try {
            ...
            // Set different flip parameters based on camera type
            switch (camera_type_) {
                case OTTO_CAMERA_OV3660:
                    camera_->SetVFlip(true);
                    camera_->SetHMirror(true);
                    ESP_LOGI(TAG, "OV3660: set VFlip=true, HMirror=true");
                    break;
                case OTTO_CAMERA_OV2640:
                default:
                    camera_->SetVFlip(true);
                    camera_->SetHMirror(false);
                    ESP_LOGI(TAG, "OV2640: set VFlip=true, HMirror=false");
                    break;
            }
            return true;
        } catch (...) {
...
public:
    OttoRobot()
        ...
        // Auto-detect hardware version (also detects camera type)
        has_camera_ = DetectHardwareVersion();
        ESP_LOGI(TAG, "Auto-detected hardware version: %s", has_camera_ ? "with camera" : "without camera");
#elif OTTO_HARDWARE_VERSION == OTTO_VERSION_CAMERA
        // Force using camera version, but still detect specific camera type
        has_camera_ = DetectHardwareVersion();
        if (!has_camera_) {
            // Detection failed, still use camera config but type is unknown
            has_camera_ = true;
            camera_type_ = OTTO_CAMERA_UNKNOWN;
            ESP_LOGW(TAG, "Forced camera version config, but could not detect camera type");
            ...
            ESP_LOGI(TAG, "Forced camera version config");
        }
#elif OTTO_HARDWARE_VERSION == OTTO_VERSION_NO_CAMERA
        // Force using no-camera version
        has_camera_ = false;
        camera_type_ = OTTO_CAMERA_NONE;
        ESP_LOGI(TAG, "Forced no-camera version config");
#else
#error \
    "Invalid OTTO_HARDWARE_VERSION setting; use OTTO_VERSION_AUTO, OTTO_VERSION_CAMERA or OTTO_VERSION_NO_CAMERA"
#endif
