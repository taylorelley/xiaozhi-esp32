...
private:
    ...
    void CheckBatteryStatus() {
        ...
        // If battery samples are insufficient, take another reading
        if (adc_values_.size() < kBatteryAdcDataCount) {
            ReadBatteryAdcData();
            return;
        }

        // Once enough samples are collected, sample battery level once every kBatteryAdcInterval ticks
        ticks_++;
        if (ticks_ % kBatteryAdcInterval == 0) {
            ReadBatteryAdcData();
        }

        // Also periodically read the temperature
    ...
    void ReadBatteryAdcData() {
        ...
        // Above the maximum value
            ...
            // Linear interpolation for the middle values
            for (int i = 0; i < 5; i++) {
        ...
        // Check whether the low-battery threshold has been reached
        if (adc_values_.size() >= kBatteryAdcDataCount) {
    ...
    void ReadTemperature() {
        ...
        if (abs(temperature - current_temperature_) >= 3.5f) {  // Trigger callback only when temperature changes by more than 3.5 C
...
public:
    ...
    bool IsCharging() {
        // If the battery is already full, stop reporting "charging"
    ...
    bool IsDischarging() {
        // Charging and discharging are not distinguished; return the opposite state
        return !is_charging_;
    }
