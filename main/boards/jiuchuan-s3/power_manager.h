...
private:
    ...
    void CheckBatteryStatus() {
        ...
        // If battery level samples are insufficient, take another reading
...
public:
    ...
    bool IsCharging() {
        ...
        if (battery_level_ == 100) {
            //ESP_LOGI(TAG, "Battery full, no longer reporting charging");
            return false;
        }
        return is_charging_;
    }
    ...
    void RegisterAllCallbacks() {
        // Register the power-state change callback (optimized version)
        power_controller_->OnStateChange([this](PowerState newState) {
            switch(newState) {
                ...
                // Disable PWR_EN
                    /* Prevent accidental wakeup after shutdown */
                    ESP_ERROR_CHECK(esp_sleep_enable_ext0_wakeup(PWR_BUTTON_GPIO, 0));
                    ESP_ERROR_CHECK(rtc_gpio_pulldown_en(PWR_BUTTON_GPIO)); // Internal pull-down
